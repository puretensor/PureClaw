...
Monitors hal@example.com via Gmail API and responds autonomously to
...
class PureClawEmailResponderObserver(Observer):
    """Autonomous email responder for hal@example.com."""
    ...
    def _search_new_emails(self) -> list[str]:
        """Search for unread emails to hal@example.com. Returns Gmail message IDs."""
        output = self._run_gmail([
            "ops", "search", "-q",
            "{to:hal@example.com to:hal@example.com} is:unread newer_than:1d"
            " -from:hal@example.com -from:ops@puretensor.ai"
            " -from:mailer-daemon",
            "-n", "10",
        ])
        msg_ids = []
    ...
    def run(self, ctx: ObserverContext = None) -> ObserverResult:
        ...
        for msg_id in new_ids:
            ...
            assessment = self.call_claude(
                f"You are {AGENT_NAME}, Deputy CTO of Pure Tensor (hal@example.com). "
